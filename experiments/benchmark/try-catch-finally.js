...
function final1() {
    arr.forEach((v, i, arr) => { arr[i] += 1 });
}
function final2() {
    arr.forEach((v, i, arr) => { arr[i] += 2 });
}
function final3() {
    arr.forEach((v, i, arr) => { arr[i] += 3 });
}
...
function test1() {
    return Math.random() > 0.5 ? { isSucc: true, data: 'aaaaa' } : { isSucc: false, errMsg: 'bbbbb' }
}
function test2() {
    if (Math.random() > 0.5) {
