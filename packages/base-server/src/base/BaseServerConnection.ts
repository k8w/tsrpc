...
> extends BaseConnection<ServiceType> {
  ...
  constructor(
    public readonly server: BaseServer<ServiceType>,
    privateOptions: PrivateBaseServerConnectionOptions
  ) {
    ...
    this.logger = new PrefixLogger({
      ...
      prefixs: privateOptions.logPrefixs ?? [
        server.chalk(`[${this.ip}] [Conn#${this.id}]`, ['gray']),
      ],
    });
  ...
  protected _errConnNotConnected(): OpResultVoid & { isSucc: false } {
    return {
      isSucc: false,
      errMsg: `The connection has been broken and no data can be sent.`,
    };
  }

  /** Close the connection immediately */
  protected override _disconnect(
    isManual: boolean,
    reason?: string
    ...
    if (dataType === 'text' && !this.server.options.json) {
      if (box.type === 'req') {
        this._sendTransportData({
          type: 'err',
          err: new TsrpcError(
            `The server disabled JSON mode, please use binary instead. (Set 'json: false' at the client)`,
      ...
      return {
        isSucc: false,
        errMsg: `Text input is not allowed when set 'json: false'`,
      };
    }
  ...
  protected _emitMsg: BaseConnection<ServiceType>['_emitMsg'] = (
    msgName,
    msg,
    msgName2,
    conn: any
