export class EventEmitter<EventType extends Record<string, any[]> = any> {
    ...
    private _addListener(type: string, value: EventEmitter<any>['_listeners'][string][0]) {
        let listeners = this._getListeners(type);
        if (!listeners.some(v => v.listener === value.listener && v.context === value.context)) {
    ...
    /** Off one */
    off<T extends string & keyof EventType>(type: T, listener: Function, context?: any): void;
    off<T extends string & keyof EventType>(type: T, listener?: Function, context?: any) {
        // Off all
        if (!listener) {
