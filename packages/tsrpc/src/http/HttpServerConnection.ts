...
> extends BaseServerConnection<ServiceType> {
  ...
  constructor(
    public readonly server: HttpServer<ServiceType>,
    privateOptions: PrivateHttpServerConnectionOptions
  ) {
    ...
    if (!contentType) {
      this.dataType =
        this.server.options.defaultDataType === 'text' &&
        this.server.options.json
          ? 'text'
          : 'buffer';
    ...
    res.on('close', async () => {
      ...
      if (req.destroyed ?? req.aborted) {
        (this.call?.logger ?? this.logger).debug(
          this.chalk('[ReqAborted]', ['debug'])
        );
        isManual = false;
      ...
      // 异常断开：直到连接关闭，也未调用过 httpRes.end 方法
        ...
        (this.call?.logger || this.logger).warn(
          'Socket closed without any response'
        );
        isManual = false;
  ...
  protected async _sendData(
    ...
    if (!this.httpRes.writable) {
      return {
        isSucc: false,
        errMsg: 'Response is not writable, you may sended response before.',
      };
    }

    return new Promise<OpResultVoid>((rs) => {
      if (
        transportData.type === 'err' &&
        transportData.err.type !== TsrpcErrorType.ApiError
      ) {
        this.httpRes.statusCode = 500;
      }
  ...
  protected override _recvApiReq(
  ...
  protected override async _recvTransportData(
    transportData: TransportData
  ): Promise<void> {
  ...
  protected override _encodeBoxText: (typeof TransportDataUtil)['encodeBoxText'] =
    (box: BoxTextEncoding & { type: 'req' }, skipSN) => {
  ...
  protected _doDecodeBoxText(data: string): OpResult<BoxTextDecoding> {
    ...
    try {
      ...
      if (this.flows.preRecvDataFlow.nodes.length) {
        this.logger.warn(
          'Cannot parse the request data to JSON. You are using "preRecvDataFlow", please check whether it transformed the data properly.',
          e
        );
      }
      return {
   ...
   */
  protected override _stringifyBodyJson: BaseConnection['_stringifyBodyJson'] =
    (bodyJson) => {
      // bodyJson must be ApiReturn<any>
      return this.options.encodeReturnText
        ? this.options.encodeReturnText(bodyJson as ApiReturn<any>)
