...
export class ApiCall<
  ...
  ApiName extends LocalApiName<Conn> = string
> {
  logger: PrefixLogger;
  ...
  async execute(): Promise<ApiReturn<Res>> {
    // Get Service
    const service = this.conn.serviceMap.name2LocalApi[this.apiName];
    if (!service) {
    ...
    // Log
    this.conn.options.logApi &&
      this.logger.info(
        this.conn.chalk('[Req]', ['info', 'bold']),
        this.conn.options.logReqBody ? this.req : ''
      );
    ...
    if (!this.conn.options.skipDecodeValidate) {
      ...
      if (!vRes.isSucc) {
        return this.error(`[ReqTypeError] vRes.errMsg`, {
          type: TsrpcErrorType.RemoteError,
          code: 'REQ_TYPE_ERR',
        });
      }
    ...
    if (!preFlow) {
      this._stopTimeout();
      this.logger.debug(
        `${this.conn.chalk('[Canceled]', ['debug'])} Canceled by preApiCallFlow`
      );
      return PROMISE_ABORTED;
    }
    if (preFlow !== this) {
      return this._internalError({
        message: 'You cannot recreate the ApiCall during Flow.',
      });
    }
  ...
  ): Promise<ApiReturn<Res>>;
  error(err: TsrpcError): Promise<ApiReturn<Res>>;
  error(
    errOrMsg: string | TsrpcError,
  ...
  protected async _sendReturn(ret: ApiReturn<Res>): Promise<ApiReturn<Res>> {
    ...
    if (this.conn.status === ConnectionStatus.Connected) {
      ...
      if (!op.isSucc) {
        ...
        if (
          this.conn.status === ConnectionStatus.Connected &&
          ret.err?.code !== 'INTERNAL_ERR'
        ) {
    ...
    if (this.conn.options.logApi) {
      if (ret.isSucc) {
        this.logger.info(
          this.conn.chalk('[Res]', ['info', 'bold']),
          this.conn.options.logResBody ? ret.res : ''
        );
      } else {
        this.logger[
          ret.err.type === TsrpcErrorType.LocalError ||
          ret.err.type === TsrpcErrorType.NetworkError
            ? 'error'
            : 'info'
        ](
          this.conn.chalk('[Err]', [
            ret.err.type === TsrpcErrorType.ApiError ? 'warn' : 'error',
            'bold',
          ]),
          ret.err
        );
    ...
    // Post Flow
    this.conn.flows.postApiCallReturnFlow.exec(
      this as this & { return: ApiReturn<Res> },
      this.logger
    );
  ...
  protected _internalError(err: { message: string }) {
    ...
    return this.error('Remote internal error', {
      code: 'INTERNAL_ERR',
      type: TsrpcErrorType.RemoteError,
      ...(this.conn.options.returnInnerError
        ? { innerErr: err.message }
        : undefined),
    });
  }
}
