...
describe("WS JSON Server & Client basic", function () {
  ...
  it("Client use JSON when server is `json: false`", async function () {
    ...
    let isErr = false
    await testApi(server, client).catch(() => {
      isErr = true
    })
  ...
  it("Client heartbeat works", async function () {
    ...
    assert.ok(
      client.lastHeartbeatLatency > 0,
      `client.lastHeartbeatLatency is ${client.lastHeartbeatLatency}`
    )
...
describe("WS JSON Flows", function () {
  ...
  it("Buffer enc/dec flow", async function () {
    ...
    client.flows.preSendDataFlow.push(v => {
      flowExecResult.client_preSendDataFlow = true
      v.data = (v.data as string).split("").reverse().join("")
      return v
    })

    client.flows.preRecvDataFlow.push(v => {
      flowExecResult.client_preRecvDataFlow = true
      v.data = (v.data as string).split("").reverse().join("")
      return v
    })

    let ret = await client.callApi("Test", { name: "xxx" })
    assert.strictEqual(flowExecResult.client_preSendDataFlow, true)
    assert.strictEqual(flowExecResult.client_preRecvDataFlow, true)
  ...
  it("client ApiReturn flow", async function () {
    ...
    const flowExecResult: { -readonly [K in keyof WsClient<any>["flows"]]?: boolean } = {}
  ...
  it("onInputBufferError", async function () {
    ...
    assert.deepStrictEqual(ret, {
      isSucc: false,
      err: new TsrpcError(
        'Input is not a valid JSON string: Unexpected token \']\', "]1,}"XXX":"... is not valid JSON',
